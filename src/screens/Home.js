...
const loadData = async ()=>{
  let response = await fetch("http://localhost:5000/api/notebookData",{
      method:"POST",
      headers: {
   ...
   setCopyItem(response[0]);
   setCopyCat(response[1]);
            ...
            ? copyCat.map((data) => {
              return (
                <div className='row mb-3'>
                  ...
                  <hr />
                  {copyItem !== [] ?
                    copyItem.filter((item) => (item.CategoryName === data.CategoryName) && (item.name.toLowerCase().includes(search.toLowerCase()))) 
                      .map((filterItems) => {
                        console.log(filterItems)
                        return (
                          <div key={filterItems._id} className="col-12 col-md-6 col-lg-3">
                            <Card copyItem = {filterItems}
                              // copyName={filterItems.name}
